...
//   p1.y = 5;
//   p1.z = 6;
//   // 자식 클래스(PointDetail)를 부모클래스(Point)로 타입캐스팅하는 것을
//   // 업캐스팅(up-casting이라 합니다.
...
void main()
{
  ...
  Unit unit1 = marine as Unit;    //객체의 형변환을 위해 as(상속받았을때만 가능)
  Unit unit2 = medic;   //현재 버전에서는 as업싱도 형변환이 가능하다.
  ...
  if (unit1 is Marine)
    ...
    /*
        ...
        하지만 다트에서는 가능하다. 마치 변수까지 오버라이딩 되는 듯한 느낌이라 생각하면 된다
    */
    // print(unit1.name); // => 단독적으로는 해당문장은 사용할수 없다.

    /*
      이 문장을 of문 블럭 외부로 옮기면 에러가 발생한다.
      is를 통해 부모타입을 자식타입으로 변환이 가능한지 확인한 후 접근을 허용하게 된다.
    */
